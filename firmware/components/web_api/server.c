...
esp_err_t web_api_server_init(void)
{
    ...
    /* Default is 8 - this project now registers 10+ routes across
     * wifi_setup_routes.c/dashboard_routes.c/history_routes.c/
     * presets_routes.c, so the default silently drops the last ones
     * registered (httpd_register_uri_handler starts failing with
     * ESP_ERR_NO_MEM once full) unless raised. */
