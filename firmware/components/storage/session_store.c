...
/* SECURITY: session_id reaches this component directly from web_api's
 * history_routes.c query-string parsing (`?id=...`), completely
 * unsanitized, and gets concatenated straight into a LittleFS path
 * (`SESSIONS_DIR "/%s.jsonl"`/".meta") and also into an HTTP
 * `Content-Disposition` header value. Without this check, a request like
 * `GET /history/detail?id=../../some/other/file` could read arbitrary
 * files on the "storage" partition (path traversal, OWASP A01), and a CRLF
 * inside `id` could inject extra HTTP headers into the export response
 * (header/response-splitting, OWASP A03). Real session ids are always
 * generated by session_store_begin_session() as
 * "session_<16 zero-padded digits>_<8 hex digits>" - restricting to
 * alnum/underscore only accepts every legitimate id and rejects any
 * '/', '.', or control character an attacker could use. */
static bool is_safe_session_id(const char *session_id)
{
    if (session_id == NULL || session_id[0] == '\0') {
        return false;
    }
    for (const char *p = session_id; *p != '\0'; p++) {
        char c = *p;
...
{
    if (out_meta == NULL || !is_safe_session_id(session_id)) {
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t session_store_get_count(uint32_t *out_count)
{
    ...
    /* BUG FIX: this used to count every non-dot file in SESSIONS_DIR,
     * which includes each session's ".meta" sidecar file alongside its
     * ".jsonl" - silently reporting roughly DOUBLE the real number of
     * recorded roasts. Reuse the same ".jsonl"-only filter count_sessions()
     * already uses elsewhere (retention/storage-limit checks), so this
     * getter actually matches what session_store_list_sessions() returns. */
    *out_count = count_sessions();
    return ESP_OK;
}
